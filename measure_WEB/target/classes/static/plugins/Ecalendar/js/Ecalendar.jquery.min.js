$.fn.ECalendar = function(config) {
	...
	var _this = $(this), ECalendar, day, imonth, iday, iyear, ilast, itimebox, select, getFullYear, getMonth, getDate, weekday, getHour, getMinutes, oper, currenttime, x, y, mx, my, timestep = config.step, drag, input, dragnum, isinit = false, pointer, callback, formatTime = config.format ? config.format
			: "yyyy-mm-dd hh:ii", formatDate = config.format ? config.format
			: "yyyy-mm-dd", tmp = [ '#ee3f2f', '#1bbc9b', '#297fb8', '#985aa6',
			'#e67f22', '#22b7b5', '#27ae61', '#7e8c8d' ];
	...
	_this
			.click(function(event) {
				var itime = _this.attr("data-ec") ? _this.attr("data-ec")
						: _this.val();
				var date = itime ? new Date(itime.replace(/-/g, "/"))
						: new Date();
				...
				if (height / 2 < top) {
					ECalendar.css({
						...
						"top" : post.top - config.offset[1]
								- ECalendar.height() - 1,
					});
				} else {
					ECalendar.css({
						"left" : post.left + config.offset[0],
						"top" : post.top + _this.outerHeight()
								+ config.offset[1],
					});
				}
	...
	_this.on("change", function() {
		var date = _this.val() ? new Date(_this.val().replace(/-/g, "/"))
				: null;
			...
			.click(
					function() {
						...
						if (index === 0)
							setData(date);
						if (index === 1)
							setData(new Date(date.getFullYear(), date
									.getMonth(), date.getDate() + 7, date
						...
						if (index === 3)
							setData(new Date(date.getFullYear(), date
									.getMonth() + 3, date.getDate(), date
									.getHours(), date.getMinutes()));
						if (index === 4)
							setData(new Date(date.getFullYear() + 1, date
	...
	day.on("mousedown", "li", function(e) {
		...
		if (config.type == 0) {
			...
			drag.attr("data-settime", (getHour < 10 ? "0" + getHour : getHour)
		...
		setData(new Date(getFullYear, getMonth - 1, getDate, getHour,
				getMinutes));
	});
	oper.click(function() {
		var li = $(this);
	...
	$(imonth).mousedown(
			function(e) {
				...
				var prehtml = '<i class="r3" data="' + (dragnum - 3) + '">'
						+ (dragnum - 3) + '</i>' + '<i class="r2" data="'
						+ (dragnum - 2) + '">' + (dragnum - 2) + '</i>'
						+ '<i class="r1" data="' + (dragnum - 1) + '">'
						+ (dragnum - 1) + '</i>';
				var apphtml = '<i class="a1" data="' + (dragnum + 1) + '">'
						+ (dragnum + 1) + '</i>' + '<i class="a2" data="'
						+ (dragnum + 2) + '">' + (dragnum + 2) + '</i>'
						+ '<i class="a3" data="' + (dragnum + 3) + '">'
						+ (dragnum + 3) + '</i>';
				...
				if (drag) {
					...
					if (drag[0].tagName == "LI") {
						if (Math.abs(my) > 1 || Math.abs(mx) > 1) {
							...
							var amin = parseInt(((atime - ahours) * 60)
									/ timestep)
									* timestep;
							...
							getHour = ahours;
							getMinutes = amin;
							drag.attr("data-settime", (ahours < 10 ? "0"
									+ ahours : ahours)
									+ ":" + (amin < 10 ? "0" + amin : amin));
							itimebox.text((ahours < 10 ? "0" + ahours : ahours)
									+ ":" + (amin < 10 ? "0" + amin : amin));
							pointer.css({
								"transform" : "rotate(" + (a - 90) + "deg)"
							});
						...
						if (drag.hasClass("imonth") && m >= 1 && m <= 12) {
							drag.children('i').each(
									function() {
										var temp = parseInt($(this)
												.attr("data"))
												+ offsetTemp;
										if (temp > 12 || temp < 1)
											$(this).hide();
	...
	$(document).mouseup(
			function() {
				if (drag) {
					if (drag[0].tagName == "LI") {
						var m = drag.text();
						setData(new Date(getFullYear, getMonth - 1, getDate,
								getHour, getMinutes));
						...
						if (drag.hasClass("imonth"))
							setData(new Date(getFullYear, m - 1, getDate,
									getHour, getMinutes));
						if (drag.hasClass("iyear")) {
							setData(new Date(m, getMonth - 1, getDate, getHour,
									getMinutes));
						}
						currenttime.animate({
							opacity : 1
...
//						+ 'px;top:50%;margin-top: -10px;" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1006" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20"><defs><style type="text/css"></style></defs><path d="M856.576 200.192v54.784h58.368V814.08H109.568V256.512h56.832v-56.32H51.2V870.4h921.6V200.192h-116.224z" fill="#666666" p-id="1007"></path><path d="M819.2 102.4h-204.288v255.488L819.2 358.4V102.4z" fill="#EF4848" p-id="1008"></path><path d="M563.712 664.064h256V716.8h-256zM563.712 512h256v51.2h-256zM461.312 200.192h102.4v56.32h-102.4zM205.312 664.064h256V716.8h-256zM205.312 512h256v51.2h-256z" fill="#666666" p-id="1009"></path><path d="M410.112 102.4H204.8v256l205.312-0.512V102.4z" fill="#EF4848" p-id="1010"></path></svg>');
	...
	function setData(date) {
		...
		if (ECalendar.data("date") != (getFullYear + "-" + getMonth)) {
			...
			for (var i = 1; i <= max; i++) {
				if (i < oneday)
					list += "<li class='other prve'>"
							+ (prevcount - (oneday - i) + 1) + "</li>";
				else if (i >= oneday && i < count + oneday) {
					if ((i - oneday + 1) == getDate)
						list += "<li class='activ' data='" + (i - oneday + 1)
								+ "' style='background:" + config.skin + "'>"
								+ (i - oneday + 1) + "</li>";
					else
						list += "<li data='" + (i - oneday + 1) + "'>"
								+ (i - oneday + 1) + "</li>";
				} else
					list += "<li class='other next'>"
							+ (i - (count + oneday) + 1) + "</li>";
			}
			day.html(list);
		}
		var now = new Date();
		var diff = new Date(getFullYear, getMonth - 1, getDate).getTime()
		...
		ilast.html(iilast);
		if (config.type == 0)
			itimebox.text((getHour < 10 ? "0" + getHour : getHour) + ":"
					+ (getMinutes < 10 ? "0" + getMinutes : getMinutes));
		var val = config.format.replace('yyyy', getFullYear).replace('mm',
				getMonth).replace('dd', getDate).replace('hh',
				getHour < 10 ? "0" + getHour : getHour).replace('ii',
				getMinutes < 10 ? "0" + getMinutes : getMinutes);
