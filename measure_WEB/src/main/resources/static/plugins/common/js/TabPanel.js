...
TabPanel.prototype = {
  ...
  init : function(){
    ...
    this.tabpanel_tab_content.addClass('custom-tab');
    this.tabpanel_tab_content.addClass('turquoise-tab');
...
//	  this.tabpanel_content.height(t_h);//用于页面高度调整
//	  this.tabpanel_content.height(t_h-this.tabpanel_tab_content.get(0).offsetHeight - 15);
	  this.tabpanel_content.height(t_h-this.tabpanel_tab_content.get(0).offsetHeight + 15);
  ...
  addTab : function(tabitem){
    ...
    if(this.maxLength!=-1 && this.maxLength<=this.tabs.length || tabitem.id == undefined){//tabitem.id == undefined判断是否有ID
	    return false;
    }
    ...
    else if(this.scrollFinish)
    {
      ...
      content.addClass(tabitem.id);//为获取iframe，给其父类加标记
  ...
  resize : function(){
      ...
      this.height = this.render.height();
  	  ...
  	  this.tabpanel_content.height(this.height-this.tabpanel_tab_content.get(0).offsetHeight - 5);
  	}
